...
def sync_uiso(struct):
    """确保所有 site 都正确继承 CIF 中的 Uiso."""
    for site in struct.sites:
        # CIF 中的字段
        if "Uiso" in site.properties:
            site.properties["_Uiso"] = float(site.properties["Uiso"])
        elif "Biso" in site.properties:
            # Biso -> Uiso
            B = float(site.properties["Biso"])
            site.properties["_Uiso"] = B / (8 * np.pi * np.pi)
        else:
            # 没有则给默认值
            site.properties["_Uiso"] = 0.01
    return struct
...
def torch_refine(exp_y: np.ndarray, profiles: List[np.ndarray], device: torch.device,
    ...
    # 这里用等间距索引作为 x，仅用于 zero-shift 插值（保持与 v12 一致）
    ...
    if global_logits is not None:
        with torch.no_grad():
            ...
            if global_logits.numel() == n_phase:
                # 只拷贝数值，不共享 Parameter 对象（保持与 v17 一致的独立优化器/参数）
    ...
    # ✅【新增】在相组合阶段对主相施加 softmax 初始偏置（与 yfsf_refine_cache.py 完全一致）
    if main_bias != 0.0:
        with torch.no_grad():
...
def _synth_profile_worker(args):
    ...
    for t0, I0, hkls in zip(t_list, I_list, hkls_all):
         # --- Debye–Waller 因子：基于每个 hkl 关联原子的 Uiso 平均值修正峰强度 
        if hkls is not None:
            dw_list = []
            theta_rad = np.deg2rad(t0 / 2.0)

            sin_theta_over_lambda2 = (np.sin(theta_rad)**2) / (wl * wl)

            for item in hkls:
                at_index = item.get("atom_index", None)
                if at_index is None:
                    continue

                Uiso = st.sites[at_index].properties.get("_Uiso", 0.01)

                # DW = exp( -8π² * Uiso * sin²θ / λ² )
                DW = np.exp(-8.0 * np.pi * np.pi * Uiso * sin_theta_over_lambda2)
                dw_list.append(DW)

            if len(dw_list) > 0:
                I0 = I0 * float(np.mean(dw_list))
        # —— PO 强度修正：多个等价 hkl 贡献的平均因子（不加权）
...
def set_cell_abc(struct: Structure, a: float, b: float, c: float) -> Structure:
    ...
    return Structure(new_lat, [s.species for s in struct.sites], [s.frac_coords for s in struct.sites])
...
def structure_with_mixed_occupancy(struct: Structure, site_index: int, occ_mix: Dict[str, float]) -> Structure:
    ...
    return Structure(lattice, new_species, fracs)
...
def outer_refine_all(x_grid: np.ndarray, y_obs: np.ndarray,
    ...
    def make_profiles(structs_dict, tpars, po_r_dict):
        U,V,W,X,Y = tpars["U"], tpars["V"], tpars["W"], tpars["X"], tpars["Y"]
        # 准备任务（把 Structure 转 dict 保证可pickle）
        items = [
            (key, st.as_dict(), x_grid, wavelength, tpars, po_axes.get(key, (0,0,1)),
             po_r_dict.get(key, 1.0), broad_base, enable_po)
            for key, st in structs_dict.items()
        ]
        # 并行计算各相 profile（包含 PO 与 TCH，与 v13 完全一致）
        with ProcessPoolExecutor(max_workers=num_workers) as ex:
            results = list(ex.map(_synth_profile_worker, items))
        # 保持与输入 key 顺序一致
        results.sort(key=lambda x: list(structs_dict.keys()).index(x[0]))
        profs = [y for _, y in results]
        return profs
                ...
                for key in list(phase_structs.keys()):
                    ...
                    for p_name in ("alpha","beta","gamma"):
                        ...
                        for sgn in (-1.0,+1.0):
                            ...
                            st_try = sync_uiso(set_cell_abc_angles(st0, a,b,c, al,be,ga))
                            structs_try = dict(phase_structs); structs_try[key]=st_try
                            score_try, yfit_try, fr_try, sf_try, r_try, _ = inner_once(structs_try, tpars, po_r,
                                                                                        freeze_scale= False,
                                                                                        stage_name=stg["name"])
